...
4. **"提供 npm 等替代安装"**（§ 12）：目前只有两条安装路径——从源码构建（三平台文档在 [`../operations/install.md`](../operations/install.md)）与中继的 Docker 镜像。npm 包与预编译产物都还没有，因为发布流水线（含可复现构建与产物摘要）尚未建立；它是审计范围里被明确列为**未完成**的一项（[`../audit-scope.md`](../audit-scope.md) § 2.4），也是 M6 之后第一个该补的工程缺口。旧 CPU 的替代路径在当前条件下是"在那台机器上从源码构建"，并已被 qemu 模拟实测覆盖。
